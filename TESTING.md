...
В cd results/test_name_scalability_by_workers_1 результаты тестового прогона на моей машине: зависимость max pps от кол-ва воркеров. Главное - pps_vs_workers.png (график), более подробно по каждой точке в папках. Имена папок соответствуют параметрам запуска. Наблюдается странное поведение, когда после 2х воркеров почему-то идет резкая деградация. Необходимо выянсить почему.
